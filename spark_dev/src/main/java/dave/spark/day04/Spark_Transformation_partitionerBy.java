...
public class Spark_Transformation_partitionerBy {
...
//        JavaPairRDD<String, Double> pairRDD = javaPairRDD.partitionBy(new HashPartitioner(2));
        JavaPairRDD<String, Double> pairRDD = javaPairRDD.partitionBy(new MyPartitioner(2));

        pairRDD.mapPartitionsWithIndex(new Function2<Integer, Iterator<Tuple2<String, Double>>, Iterator<Tuple2<String, Double>>>() {
            @Override
            public Iterator<Tuple2<String, Double>> call(Integer index, Iterator<Tuple2<String, Double>> iterator) throws Exception {
                List<Tuple2<String, Double>> list = new ArrayList<>();
                while (iterator.hasNext()) {
                    Tuple2<String, Double> sr = iterator.next();
                    System.out.println("partition" + index + ":" + sr);
                    list.add(sr);
                }
                return list.iterator();
        ...
        pairRDD.collect().forEach(new Consumer<Tuple2<String, Double>>() {
            @Override
            public void accept(Tuple2<String, Double> stringDoubleTuple2) {
                System.out.println("===end==" + stringDoubleTuple2);
            }
        });
